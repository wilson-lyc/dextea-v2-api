...
public class RoleServiceImpl implements RoleService {
    ...
    @Override
    public DexteaApiResponse<List<StaffModel>> getRoleStaffList(Long id) {
        MPJLambdaWrapper<StaffRole> wrapper=new MPJLambdaWrapper<StaffRole>()
                .select(StaffRole::getStaffId)
                .eq(StaffRole::getRoleId,id);
        List<Long> staffIds=staffRoleMapper.selectJoinList(Long.class,wrapper);
        if(staffIds.isEmpty()){
            return DexteaApiResponse.success(List.of());
        }
        List<StaffModel> staffList=staffFeign.getStaffInIds(staffIds);
        return DexteaApiResponse.success(staffList);
    }
}
