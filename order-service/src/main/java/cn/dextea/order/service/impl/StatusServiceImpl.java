...
import cn.dextea.order.code.UpdateStatusMode;
...
public class StatusServiceImpl implements StatusService {
    ...
    @Override
    public DexteaApiResponse<Void> waitPick(String id, String mode) {
        // 查询订单
        ...
        orderMapper.update(updateWrapper);
        // 默认模式 - 取餐叫号+发送订单数据
        if(Objects.isNull(mode) || mode.equals(UpdateStatusMode.DEFAULT.getValue())){
            // 取餐叫号
    ...
    @Override
    public DexteaApiResponse<Void> done(String id, String mode) {
        // 查询订单
        Order order = orderMapper.selectById(id);
        if (Objects.isNull(order)) {
            return DexteaApiResponse.fail(OrderErrorCode.ORDER_NOT_FOUND.getCode(),
    ...
    @Override
    public DexteaApiResponse<Void> refund(OrderRefundRequest data, String mode) {
        // 验证密码
